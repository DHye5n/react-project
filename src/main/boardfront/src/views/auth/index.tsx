...
export default function Authentication() {
            ...
            <div className='auth-card'>
                <div className='auth-card-box'>
                    <div className='auth-card-top'>
                        ...
                        <InputBox ref={emailRef} label='이메일 주소' type='text'
                                  placeholder='이메일 주소를 입력해주세요.' error={error} value={email}
    ...
    const SignUpCard = () => {
        ...
        // event handler: 개인 정보 동의 체크 박스 변경 이벤트 처리
        const onAgreedPersonalClickHandler = () => {
            setAgreedPersonal(!agreedPersonal);
            setAgreedPersonalError(false);
        }

        // event handler: 비밀번호 클릭 이벤트 처리
        const onPasswordButtonClickHandler = () => {
        ...
        // event handler: 비밀번호 확인 클릭 이벤트 처리
        ...
        const onSignUpButtonClickHandler = () => {
            ...
            const hasZonecode = zonecode.trim().length > 0;
            if (!hasZonecode) {
                setZonecodeError(true);
                setZonecodeErrorMessage('우편번호를 선택해주세요.');
            }
            ...
            const hasAddressDetail = addressDetail.trim().length !== 0;
            if (!hasAddressDetail) {
                setAddressDetailError(true);
                setAddressDetailErrorMessage('상세주소를 입력해주세요.');
            }
            ...
            const requestBody: SignUpRequestDto = {
                email, password, nickname, telNumber,zonecode , address, addressDetail, agreedPersonal
            };
            ...
            <div className='auth-card'>
                <div className='auth-card-box'>
                    <div className='auth-card-top'>
                        ...
                        {page === 1 && (
                            <>
                                <InputBox ref={emailRef} label='이메일 주소*' type={'text'} error={isEmailError} placeholder='이메일 주소를 입력해주세요.'
                                          value={email} onChange={onEmailChangeHandler} message={emailErrorMessage} onKeyDown={onEmailKeyDownHandler}/>
                                <InputBox ref={passwordRef} label='비밀번호*' type={passwordType} error={isPasswordError} placeholder='비밀번호를 입력해주세요.'
                                          value={password} onChange={onPasswordChangeHandler} message={passwordErrorMessage}
                                          icon={passwordButtonIcon} onButtonClick={onPasswordButtonClickHandler} onKeyDown={onPasswordKeyDownHandler} />
                                <InputBox ref={passwordCheckRef} label='비밀번호 확인*' type={passwordCheckType} error={isPasswordCheckError} placeholder='비밀번호를 다시 입력해주세요.'
                                          value={passwordCheck} onChange={onPasswordCheckChangeHandler} message={passwordCheckErrorMessage}
                                          icon={passwordCheckButtonIcon} onButtonClick={onPasswordCheckButtonClickHandler} onKeyDown={onPasswordCheckKeyDownHandler} />
                            </>
                        )}
                        {page === 2 && (
                            <>
                                <InputBox ref={nicknameRef} label='닉네임*' type='text' error={isNicknameError} placeholder='닉네임을 입력해주세요.' value={nickname}
                                          onChange={onNicknameChangeHandler} message={nicknameErrorMessage} onKeyDown={onNicknameKeyDownHandler} />

                                <InputBox ref={telNumberRef} label='핸드폰 번호*' type='text' error={isTelNumberError} placeholder='핸드폰 번호를 입력해주세요.' value={telNumber}
                                          onChange={onTelNumberChangeHandler} message={telNumberErrorMessage} onKeyDown={onTelNumberKeyDownHandler} />

                                <InputBox ref={zonecodeRef} label='우편번호*' type='text' error={isZonecodeError} placeholder='우편번호' value={zonecode}
                                          onChange={onZonecodeChangeHandler} message={zonecodeErrorMessage} icon='expand-home-icon' onButtonClick={onAddressButtonClickHandler}
                                          onKeyDown={onAddressKeyDownHandler} />

                                <InputBox ref={addressRef} label='주소*' type='text' error={isAddressError} placeholder='주소 찾기' value={address}
                                          onChange={onAddressChangeHandler} message={addressErrorMessage}  onKeyDown={onAddressKeyDownHandler} />

                                <InputBox ref={addressDetailRef} label='상세 주소*' type='text' error={isAddressDetailError} placeholder='상세 주소를 입력해주세요.' value={addressDetail}
                                          onChange={onAddressDetailChangeHandler}  onKeyDown={onAddressDetailKeyDownHandler} message={addressDetailErrorMessage} />
                            </>
                        )}
                    </div>
                    <div className='auth-card-bottom'>
                        ...
                        <div className='auth-description-box'>
                            <div className='auth-description'>{'이미 계정이 있으신가요? '}
                                <span className='auth-description-link' onClick={onSignInLinkClickHandler}>{'로그인'}</span></div>
                        </div>
                    </div>
                </div>
            </div>
    ...
    // render: 인증 화면 Component Rendering
    return (
        <div id='auth-wrapper'>
                ...
                <div className='auth-jumbotron-box'>
                    <div className='auth-jumbotron-contents'>
                        ...
                        <div className='auth-jumbotron-text-box'>
                            ...
                            <div className='auth-jumbotron-text'>{'DgmFit 게시판입니다.'}</div>
                        </div>
                    </div>
                </div>
